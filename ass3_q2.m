...
for i=2:size(t_laser,1)
    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            % Find particle pose in pixels
            x_pix = floor((x_particle(n)-ogxmin)/ogres);
            y_pix = floor((y_particle(n)-ogymin)/ogres);
            
            % Like in Q1, find indices of pixels in ray path to particle
            
            % First, get total angle of robot + laser
            phi = theta_particle(n) + angles(j);
            
            y_laser_px = y_laser(i, j)/ogres;
            ...
            % Add particle cell pose values
            x_idx = x_idx + x_pix;
            y_idx = y_idx + y_pix;
            
            % Get predicted laser range distance
            max_idx = 1;
            
            for x=1:size(x_idx, 2)
                if x_idx(x) < 1 || y_idx(x) < 1
                    continue
                end
                if ogp(y_idx(x),x_idx(x)) >= 0.5
                    max_idx = x;
                    break
                end
            end
            
            x_dist = (x_idx(max_idx)*ogres) + ogxmin;
            y_dist = (y_idx(max_idx)*ogres) + ogymin;
            pred_range = sqrt((x_dist - x_particle(n))^2 + (y_dist - y_particle(n))^2);
            
            % Update particle filter weight with a Gaussian PDF
            gaussian_pdf = exp((-1*(y_laser(i, j) - pred_range)^2)/(2*laser_var))*(sqrt(laser_var)*sqrt(2*pi))^-1;
            w_particle(n) = w_particle(n)*w_gain*gaussian_pdf;
